...
const todos = ref(JSON.parse(localStorage.getItem(STORAGE_TODO_KEY)) || [])
// computed
...
watchEffect(function () {
    localStorage.setItem(STORAGE_TODO_KEY, JSON.stringify(todos.value))
})
...
function checkTodoIncludeFilterKey(item) {
...
<template>
    <div class="p-todoList">
        ...
        <div class="p-todoList__form">
            ...
            >
            <button class="p-todoList__form-btn" :disabled="checkIsEmptyAdd" @click="addTodo()">
                Add
            </button>
        </div>
        <div class="p-todoList__form">
            <input class="p-todoList__form-input" placeholder="Search for todo by title" v-model="filterKey">
            <button v-show="filterKey.length != 0" class="p-todoList__form-btn" @click="deleteFilterKey()">x</button>
        </div>
        <draggable 
            v-model="todos" 
            tag="ul" 
        ...
        >
            <template #item="{ element: item }">
                <li 
                    ...
                    v-show="checkTodoIncludeFilterKey(item)"
                >
                    <div class="p-todoList__item-view">
                        <input class="p-todoList__item-toggle" type="checkbox" v-model="item.done">
